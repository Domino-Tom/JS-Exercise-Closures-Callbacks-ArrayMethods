...
function processSum(numberList, callback) {
  let sum = numberList.reduce((total, current) => total + current, 0);
  return callback(sum);
}
...
function tallyUpDonations(runners) {
  return runners.map(item => item.donation).reduce((total, current) => total + current, 0);
}
